...
async def list_knowledge(namespace: Optional[str] = None) -> list[dict]:
    _cols_with_user = """k.id, k.namespace, k.container_name, k.target_tables,
    ...
    async with get_conn() as conn:
        if namespace:
            rows = await conn.fetch(
                f"SELECT {_cols_with_user} FROM ops_knowledge k LEFT JOIN ops_user u ON k.created_by_user_id = u.id WHERE k.namespace = $1 ORDER BY k.created_at DESC",
                namespace,
            )
        else:
            rows = await conn.fetch(
                f"SELECT {_cols_with_user} FROM ops_knowledge k LEFT JOIN ops_user u ON k.created_by_user_id = u.id ORDER BY k.namespace, k.created_at DESC"
            )
...
async def get_knowledge_namespace(knowledge_id: int) -> Optional[str]:
    """리소스의 namespace 반환 (네임스페이스 소유 파트 기반 권한 체크용)."""
    async with get_conn() as conn:
        return await conn.fetchval(
            "SELECT namespace FROM ops_knowledge WHERE id = $1", knowledge_id
        )
...
async def create_glossary(
    ...
    *, created_by_part: Optional[str] = None, created_by_user_id: Optional[int] = None,
    ...
    async with get_conn() as conn:
        row = await conn.fetchrow(
            """
            INSERT INTO ops_glossary (namespace, term, description, embedding, created_by_part, created_by_user_id)
            VALUES ($1, $2, $3, $4::vector, $5, $6)
            RETURNING id, namespace, term, description, created_by_part, created_by_user_id
            """,
            namespace, term, description, str(embedding), created_by_part, created_by_user_id,
        )
    return dict(row)


async def list_glossary(namespace: Optional[str] = None) -> list[dict]:
    _cols = "g.id, g.namespace, g.term, g.description, g.created_by_part, g.created_by_user_id, u.username AS created_by_username"
    async with get_conn() as conn:
        if namespace:
            rows = await conn.fetch(
                f"SELECT {_cols} FROM ops_glossary g LEFT JOIN ops_user u ON g.created_by_user_id = u.id WHERE g.namespace = $1 ORDER BY g.id DESC",
                namespace,
            )
        else:
            rows = await conn.fetch(
                f"SELECT {_cols} FROM ops_glossary g LEFT JOIN ops_user u ON g.created_by_user_id = u.id ORDER BY g.id DESC"
            )
    return [dict(r) for r in rows]
...
async def get_glossary_namespace(glossary_id: int) -> Optional[str]:
    """용어집의 namespace 반환 (네임스페이스 소유 파트 기반 권한 체크용)."""
    async with get_conn() as conn:
        return await conn.fetchval(
            "SELECT namespace FROM ops_glossary WHERE id = $1", glossary_id
        )
