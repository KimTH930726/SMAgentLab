...
async def _migrate_system_tables(conn) -> None:
    ...
    await conn.execute("""
        ...
        ('sql2_parse',          'SQL 질문 분석',           $5,  'Text2SQL 파이프라인 1단계: intent/difficulty/entities 추출 프롬프트. {{question}} 플레이스홀더 필수', 'text2sql'),
        ('sql2_parse_system',   'SQL 질문 분석 시스템',    $6,  'Text2SQL parse 단계 시스템 프롬프트',                                    'text2sql'),
        ('sql2_generate',       'SQL 생성',                $7, 'Text2SQL 파이프라인 3단계: SQL 생성 프롬프트. {{question}}·{{schema}} 등 플레이스홀더 필수', 'text2sql'),
        ('sql2_generate_system','SQL 생성 시스템',          $8, 'Text2SQL generate 단계 시스템 프롬프트',                                 'text2sql'),
        ('sql2_fix',            'SQL 자동 수정',           $9, 'Text2SQL 파이프라인 5단계: 검증 실패 SQL 수정 프롬프트. {{sql}}·{{errors}}·{{schema}} 필수', 'text2sql'),
        ('sql2_fix_system',     'SQL 자동 수정 시스템',    $10, 'Text2SQL fix 단계 시스템 프롬프트',                                      'text2sql'),
        ('sql2_summarize',      'SQL 결과 요약',           $11, 'Text2SQL 파이프라인 7단계: 실행 결과 요약+차트 추천 프롬프트. {{question}}·{{sql}}·{{result_preview}}·{{columns}} 필수', 'text2sql'),
        ('sql2_summarize_system','SQL 결과 요약 시스템',   $12, 'Text2SQL summarize 단계 시스템 프롬프트',                                 'text2sql')
