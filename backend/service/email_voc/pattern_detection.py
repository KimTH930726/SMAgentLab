...
#    접근 불가로 미검증) 도입 시 재검토.
# ④ 2026-09-15 — v2.72 임베딩 교체(mpnet-768→KURE-v1) 후 위 0.70이 재검증 없이
#    방치돼 있던 걸 발견(VOC 반복패턴 임계치와 같은 문제, project_voc_threshold_
#    recalibration 메모리 참고). 이 판정은 LLM 최종검증이 있어야 정확한 재보정이
#    가능한데 지금 사내 LLM 게이트웨이가 월간 크레딧 초과로 막혀있어(2026-09
#    소진) 대량 검증 불가 — 대신 수동으로 고른 확실한 정/오탐 8쌍(LLM 미사용,
#    직접 판단)만으로 방향만 확인: 확실한 오탐 0.40~0.44, 확실한 정탐 0.60~0.70.
