# 사내 이메일 크롤링 기반 선제적 오류 분석 채널 — 기획/설계 문서

> 작성일: 2026-07-08
> 현재 버전: v2.28 기준
> 상태: **기획 단계 (미착수)** — dev_0 브랜치에서 검토
> 목적: 채팅과 별개의 새 "채널"로, 특정 기간 동안의 사내 이메일을 수집해 건별로 RAG+LLM 오류 분석을 수행하고 결과를 메일/Teams로 선제 통보하는 기능의 실현 가능성과 단계별 실행 계획을 정리한다

---

## 1. 배경 및 요구사항

현재 시스템은 **사용자가 채팅으로 질문 → RAG 근거 기반 답변**을 받는 pull 방식만 존재한다.

추가로 검토 중인 것은 push 방식의 새 채널이다:

1. 관리자가 **특정 기간**(예: 최근 1주)을 지정
2. 그 기간 동안의 **사내 이메일을 크롤링**
3. 이메일을 건별로 구분 — 대부분 시스템 오류 관련 문의(VOC 성격)로, 실제 원인이 **사용자 실수인지 시스템 문제인지** 판단이 필요
4. 각 이메일 내용에 대해 **RAG(지식 검색) + LLM**으로 원인 분석 및 (오류가 맞다면) 해결 방안을 도출
5. 분석 결과를 **사내 메일 또는 Teams로 발송** — 선제 대응

즉 "채팅 채널"과 별도로 "이메일 배치 분석 채널"을 새로 만드는 구조다.

---

## 2. 현재 자산 감사 — 재사용 가능한 것 vs 신규 구축 필요한 것

코드베이스 전수 조사 결과:

| 구성 요소 | 현황 | 재사용 가능 여부 |
|---|---|---|
| 이메일 읽기(크롤링) | IMAP/SMTP/MS Graph API 등 이메일 관련 코드/의존성이 **전무** | ❌ 처음부터 구축 |
| 이메일 발송 | SMTP 등 발송 코드 없음 | ❌ 신규 구축 |
| Teams 발송 | 없음. 기존 "Teams 연동"(`backend/service/teams/`, `backend/agents/knowledge_rag/ingestion/teams_crawler.py`)은 OpsNavHelper.exe가 사용자의 실제 로그인 브라우저 세션에서 단명 토큰(IC3)을 캡처해 **읽기 전용으로 대화 이력을 크롤링**하는 구조. 서버 자체 서비스 계정이 없고(`teams/router.py`: "서버사이드 Playwright/subprocess는 사용하지 않는다"), 무인 자동 발송에 재사용 불가 | ❌ 별도 발송 채널(Incoming Webhook/봇 앱 등록) 신규 구축 필요 |
| 배치/스케줄링 | cron·APScheduler·Celery 등 주기 실행 인프라 **전무**. `main.py`의 `lifespan`은 서버 기동 시 1회성 초기화(DB pool, 임베딩/리랭커 모델 로드, LLM health check)만 수행 | ❌ 신규 구축 필요 (단, "관리자가 기간 입력 후 수동으로 분석 시작 버튼 클릭" 방식이면 스케줄러 없이 v1 구현 가능) |
| 이메일 건별 RAG 분석 | `backend/agents/knowledge_rag/knowledge/retrieval.py`의 `search_knowledge()` + `backend/service/llm/base.py`의 `LLMProvider.generate(context, question, ...)` — 채팅 SSE/DB 스키마와 분리된 순수 비동기 함수라 "텍스트 입력 → 근거 기반 답변" 용도로 그대로 재사용 가능 | ✅ 거의 그대로 재사용 |
| 외부 API 호출 실행기 | `backend/agents/mcp_tool/agent.py`의 `_execute_http_call` — 관리자가 등록한 외부 HTTP API(메서드/URL/헤더/파라미터)를 호출하는 범용 실행기, 타임아웃/응답 크기 제한/감사 로그 포함. 현재는 LLM 도구 선택 흐름에서만 호출되지만 함수 자체는 독립 호출 가능 | ✅ 사내에 메일 발송 API나 Teams Webhook URL이 이미 존재한다면 이를 MCP 도구처럼 등록해 배치 잡에서 직접 호출하는 방식으로 재사용 가능 |

**결론**: "이메일을 분석하는" 핵심 로직은 기존 RAG 파이프라인을 그대로 타면 되므로 기술적으로 제일 쉬운 부분이다. 반면 "이메일을 읽는 것"과 "결과를 내보내는 것"은 완전히 새로운 외부 연동이며, 이 두 가지는 **사내 인프라/보안 정책에 의존**하므로 코드 작업보다 조직적 확인이 선행되어야 한다.

---

## 3. 미해결 질문 (구현 착수 전 확인 필요)

| # | 질문 | 왜 중요한가 |
|---|------|------|
| Q1 | 읽을 메일함이 **개인 메일함**인가 **공용 접수함(alias)**인가? | 개인함 전수 크롤링은 권한/프라이버시 문제가 큼. 장애 접수용 공용함이라면 서비스 계정 하나로 해결 가능해 훨씬 단순해짐 |
| Q2 | 사내 메일 시스템이 **Exchange Online(M365)** 인가 **온프레미스 Exchange**인가? | M365면 MS Graph API + 앱 등록(서비스 프린시펄)이 정석 경로 — 보안팀 승인/앱 등록 절차 필요. 온프레미스면 IMAP/EWS 프로토콜로 비교적 가볍게 접근 가능 |
| Q3 | 발송 채널로 어떤 게 이미 사내에 구축돼 있는가? (메일 발송용 SMTP 릴레이, Teams Incoming Webhook, 사내 알림 봇 등) | 이미 있는 API가 있다면 MCP 도구 실행기를 그대로 재사용해 개발 범위를 크게 줄일 수 있음 |
| Q4 | "시스템 오류로 판단" 시 **누구에게, 어떤 형식으로** 알릴 것인가? (담당 파트 전체 vs 특정 채널, 요약 vs 원문 포함) | 수신 대상 설계에 따라 발송 로직/개인정보 처리 범위가 달라짐 |
| Q5 | 오분류(사용자 실수를 시스템 오류로 잘못 판단 등) 시 리스크 허용 수준은? | 자동 발송 범위(전체 자동 vs 사람 확인 후 발송)를 결정하는 핵심 기준 |

---

## 4. 초안 아키텍처 (Phase 1 기준)

```
[관리자 어드민 화면]
  기간 선택 (from ~ to) → "분석 시작" 클릭
        │
        ▼
[이메일 수집기 — 신규]
  IMAP/EWS 또는 MS Graph API로 지정 기간 이메일 조회
  건별로 제목/본문/발신자/수신시각 파싱
        │
        ▼
[건별 RAG 분석 — 기존 파이프라인 재사용]
  각 이메일 본문을 질문처럼 취급
    → retrieval.search_knowledge(namespace, embedding)  (기존 함수)
    → LLMProvider.generate(context=검색결과, question=이메일 본문)  (기존 함수)
    → 분류: 시스템 오류 / 사용자 실수 / 판단 보류
    → 오류로 판단 시 해결 방안 초안 생성
        │
        ▼
[결과 저장 — 신규 테이블]
  이메일별 원문 + 분류 + 근거 지식 + 해결 방안 초안을 DB에 저장
        │
        ▼
[어드민 결과 화면 — 신규 UI]
  분석 결과 리스트 (건별 카드/테이블)
  Phase 1: 여기서 종료 — 발송 없음, 사람이 눈으로 확인
  Phase 2: 관리자가 "발송" 버튼 클릭 → 신규 발송 모듈(메일/Teams) 호출
  Phase 3: 확신도 높은 케이스 자동 발송 + 스케줄러로 주기 실행
```

---

## 5. 단계별 실행 계획

### Phase 1 — 수집 + 분석 (발송 없음, 사람 검토 전제)
- 목표: 새 인프라(스케줄러/발송 채널) 없이, 기존 RAG 파이프라인 재사용만으로 "이메일 건별 분석 결과를 어드민 화면에서 확인"할 수 있게 한다.
- 범위:
  - 이메일 수집기 신규 구현 (Q1·Q2 답변에 따라 IMAP 또는 Graph API 중 택1)
  - 이메일 건별 분석 배치 함수 (`search_knowledge` + `generate` 재사용)
  - 결과 저장용 신규 테이블 (예: `ops_email_analysis` — 원문, 분류, 근거 지식 ID, 해결 방안, 상태)
  - 관리자 화면: 기간 입력 → 수동 트리거 → 결과 리스트/상세 모달
- 이 단계는 Q3~Q5(발송 채널·정책)가 확정되지 않아도 착수 가능

### Phase 2 — 반자동 발송
- 목표: 분석 결과 중 관리자가 확인 후 승인한 건만 실제로 메일/Teams로 발송
- 전제조건: Q3(발송 채널 확보), Q4(수신 대상/형식) 확정
- 범위: 발송 모듈 신규 구현 (SMTP 또는 사내 발송 API 호출 — 가능하면 MCP 도구 실행기 재사용), 어드민 화면에 "발송" 액션 추가

### Phase 3 — 완전 자동화
- 목표: 확신도 높은 케이스에 한해 사람 개입 없이 자동 발송, 스케줄러로 주기 실행
- 전제조건: Phase 2 운영 데이터로 오분류율 측정 후 허용 가능한 임계값 확보 (Q5)
- 범위: 배치 스케줄러 도입(APScheduler 등), 확신도 임계값 기반 자동/수동 분기

---

## 6. 참고 — 기존 Human-in-the-loop 원칙과의 정합성

이 시스템은 지식 등록 시에도 "LLM이 청킹/카테고리를 제안하되 최종 확정은 사람이 한다"는 원칙을 일관되게 적용하고 있다(`docs/architecture.md` 참고). 이메일 분석 결과를 검증 없이 즉시 조직 전체에 발송하는 것은 이 원칙과 배치되므로, Phase 1~2에서 사람 확인 단계를 두는 것을 권장한다.

---

## 7. 신규 발견 — 사내 blossomai(AI 업무비서) API 활용 가능성 (2026-07-08)

> ⚠️ **중요 — 이 절 전체는 "가능성 확인용 참고 자료"이지 채택 가능한 연동 경로가 아니다.**
> 아래 API는 blossomai 팀이 공식 제공/문서화한 API가 아니라, 브라우저 개발자도구로 관찰한
> **비공식 내부 엔드포인트**다. 공개 계약이 없으므로 예고 없이 응답 구조·인증 방식·경로 자체가
> 바뀔 수 있고, 우리가 이를 자동화된 배치로 반복 호출하는 것은 blossomai 팀 입장에서
> **승인되지 않은 트래픽**으로 취급될 수 있다(레이트리밋/차단 대상, 보안 검토 대상이 될 가능성).
> **이 엔드포인트를 코드에서 직접 호출하는 것은 금지 — blossomai 담당팀에 공식 문의해
> 정식 API 또는 서비스 계정을 제공받기 전까지는 "사내에 이런 기능이 이미 존재한다"는
> 증거 자료로만 활용한다.**

브라우저 DevTools 캡처로 다음 사내 (비공식) 엔드포인트를 확인함:

```
GET https://blossomai.shinsegae.com/api/work-assistant/summary/{employee_email}
    ?email_days=7&calendar_days=7&include_ai_summary=true
Authorization: Bearer {JWT}   ← blossomai 로그인 세션 토큰
```

- 사내에 이미 존재하는 "AI 업무비서" 서비스(blossomai, `/daily-briefing` 화면에서 호출)로,
  **특정 기간의 이메일/캘린더를 읽어와 AI 요약까지 포함해 반환**하는 기능이 이미 구현돼 있음.
- Bearer 토큰의 JWT 클레임은 로그인한 개인 사번·이메일·부서 정보를 담은 **개인 세션 토큰**(만료시간 짧음)이며,
  서비스 계정/앱 등록 형태의 credential은 아닌 것으로 보임 (미확인).

### 의미

이게 사실이라면 §2의 "이메일 읽기(크롤링)" 항목을 처음부터 구축하지 않고, **blossomai API를 호출해 데이터를 받아오는 방식으로 대체**할 수 있는 가능성이 있음 — Q1·Q2(메일함 종류, 메일 시스템 연동 방식)를 사실상 우회.

### 이 경로를 실제로 채택하기 전에 확인해야 할 것

| # | 확인 사항 | 왜 중요한가 |
|---|---|---|
| Q6 | 응답 JSON이 **이메일 건별 구조化 데이터**(제목/본문/발신자/시각)를 주는가, 아니면 뭉뚱그린 AI 요약 문단 하나만 주는가 | 우리 기능은 이메일 건별 분류가 필요 — 후자면 그대로 쓰기 어려움 |
| Q7 | 이 인증이 **개인 로그인 세션에만** 묶여있는가, blossomai 쪽에 **서비스 계정/앱 등록으로 무인 호출**이 가능한 옵션이 있는가 | Phase 1을 "관리자 개인 세션으로 자기 메일함 분석"(Teams 연동과 동일 패턴)으로 갈지, 진짜 무인 배치로 갈지가 갈림 |
| Q8 | `{email}` 경로가 **본인 메일함 외에 공용/부서 접수함도 조회 가능**한가 | 원래 구상(장애접수용 공용 메일함 분석)과 범위가 맞는지 확인 필요 |
| Q9 | **(선행 조건, 최우선)** blossomai 담당팀에 정식으로 문의해 이 기능을 위한 **공식 API/서비스 계정을 제공받을 수 있는가** | 지금은 비공식 엔드포인트라 채택 자체가 불가 — 이걸 공식화하는 것이 전제조건이지 "확인하면 좋은 것"이 아님 |

**결론**: 이 발견은 "사내에 이메일+AI 요약 연동이 기술적으로 이미 가능하다"는 것을 보여주는 참고 자료일 뿐, 그 자체로 채택 가능한 연동 경로는 아니다. **다음 행동은 코드 작업이 아니라 blossomai 담당팀에 공식 문의하는 것** — 정식 API/서비스 계정을 제공받을 수 있는지, 가능하다면 Q6~Q8(본문 전체 여부, 인증 지속가능성, 공용함 조회 가능 여부)을 그 팀과 함께 확인한다. 공식 경로가 열리기 전까지는 §2·§4의 자체 구축 경로(IMAP/Graph API)를 기본안으로 유지한다.

### 7.1 실제 응답 샘플 분석 (2026-07-08)

실제 응답 body를 확인함. 주요 구조:

```
data.emails.recent_emails[]  — 이메일 건별 배열: subject, preview, from, from_email,
                                received_date, is_read, importance, has_attachments
data.calendar.*_schedule[]   — 일정 건별 배열: subject, start/end_datetime, location,
                                organizer, attendees_count, teams_link
data.ai_summary.summary_text — blossomai 자체 LLM이 생성한 마크다운 브리핑
                                (긴급 항목 / 오늘 일정 / 할 일 / 답장대기 / 업무부하 제안)
consent_info: {has_consent, user_id, service_type: "work_assistant"}
```

**Q6 (건별 구조 여부) — 부분 확인됨(✅ 건별 데이터 있음, ⚠️ 단서 있음)**
- `recent_emails[]`는 이메일 건별로 나뉘어 있어 원하는 그레인은 확보됨.
- 다만 본문이 `preview`(짧은 미리보기 스니펫)만 있고 전체 본문 필드가 안 보임. 우리 목적(원인 분석 + 해결 방안 도출)엔 본문 전체가 필요할 가능성이 높음 — **`preview`의 실제 길이/이 API가 전체 본문도 주는 옵션이 있는지 확인 필요** (미확인 상태로 남김).

**Q8 (공용 메일함 조회 가능 여부) — 부정적 신호**
- `consent_info.has_consent`가 응답에 포함된 것으로 보아, 이 서비스는 **개인이 자기 메일함 접근에 개별 동의(consent)한 구조**로 추정됨. 원래 구상한 "장애접수용 공용 메일함(VOC함)"을 이 API로 바로 조회할 수 있을지는 불확실 — 별도 계정으로 공용함을 blossomai에 등록·동의시키는 절차가 필요할 가능성. **미해결로 남겨두고 blossomai 담당팀 확인 필요**.

**부수 발견 — 기능 포지셔닝 재정의**
`ai_summary`가 이미 WhaTap(모니터링 툴) 경고 메일을 읽고 "MySQL 에이전트 비활성 — 확인 및 복구 필요" 같은 액션 아이템을 생성하고 있음. 단, 이는 범용 LLM 요약이지 **우리 내부 지식(runbook)에 근거한 구체적 해결 방안은 아님**. 즉:
- blossomai = "무엇을 봐야 하는지" 감지·요약 (이미 있음, 잘 함)
- 우리 기능 = 그 원본 이메일을 받아 **우리 RAG로 "왜 그런지 + 구체적으로 어떻게 고치는지"를 붙이는 것** (아직 없는 부분, 우리가 채울 차별점)

이 재정의에 따라 이메일 자체를 처음부터 크롤링하기보다, **blossomai 응답을 입력으로 받아 우리 RAG+LLM을 얹는 방향**이 유력한 대안으로 부상함. 단, 본문 전체 확보 여부(Q6 세부)와 공용함 접근 여부(Q8)가 확정되기 전까지는 §2·§4의 자체 구축 경로를 기본안으로 유지.
