...
**결론**: "하이브리드가 무조건 낫다"는 가설은 기각. narrative에서는 뚜렷한 우위가 있지만, param
유형에서 벡터 폴백 부재가 실제 약점으로 확인됐다 — 위 v2 후보 수정 없이 지금 스키마를
`agent.py`/`retrieval.py`에 그대로 편입하면 오히려 자연어 param 질의에서 퇴보할 수 있다는 뜻.
편입 전에 이 폴백 보완이 선행돼야 한다.
...
- [x] **골든셋 v1 자동 생성(2026-09-04)** — 태훈에게 수작업 요청 대신, 실 DB(온라인스토어+
      ...
      §9에서 신뢰도 확인됨).
- [x] **Track 2 비교군 A/B 실행(2026-09-04)** — §4-3에 결과. "하이브리드가 무조건 낫다" 가설
      기각, param 유형에서 벡터 폴백 부재가 실제 약점으로 확인(전체 item의 36%가 벡터 색인이
      아예 없음). agent.py/retrieval.py 편입은 이 폴백 보완 전엔 보류 권장(§4-2)
