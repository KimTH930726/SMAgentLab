...
> 상태(2026-09-07 최종 갱신): **v1 파이프라인+API 전부 구현·실 HTTP E2E 검증 완료, Track 2
> 실행 완료, 메인 채팅에 편입(1·2단계) + 정책 근거 카드 1건 압축까지 완료**(`service/policy/`
> — excel_parser/decompose/service/search/unresolved_report/browse/track2/router, 테스트
> 343개). 실 데이터: 스타벅스
...
> 비교 → 1차는 param 유형에서 B가 짐(벡터 폴백 부재), 원인 수정(item마다 최소 1개 벡터 청크
> 보장) 후 재측정하니 4개 유형 전부 B 우세로 확정(전체 정답률 57.3%→75.3%). `POST
...
- [x] **agent.py 편입 1단계(2026-09-06)** — Track 2로 B(하이브리드) 우세 확정 후 실행.
      `stream_chat()`이 `rag_knowledge` 검색과 함께 `search_policy()`도 호출해 `doc_context`에
      정책 데이터를 텍스트로 병합. `has_policy_data(namespace)`로 정책 없는 네임스페이스는
      매 턴 쿼리 스킵, `search_policy(query_vec=...)`로 중복 임베딩 방지. 인용 카드 UI는 아직
      안 건드림(2단계 후보, 실사용 피드백 대기) — `retrieval.py` 자체는 안 건드림(정책
...
- [x] **정책 근거 카드 1건 압축(2026-09-07, 같은 날)** — 2단계 배포 직후 "근거가 너무
      많이 보인다(지식 3개+정책 10개), 원문 1건만" + "10개를 참조해서 답변 만든 거냐"는
      피드백(답은 "그동안은 그랬다"). **1차 시도(바로 폐기)**: 검색 직후 벡터 점수 1위
      후보 하나로 LLM 컨텍스트까지 줄이는 `select_top_policy_hit()`을 만들었으나, 실
      E2E에서 "장바구니 최대 개수" 질문의 벡터 점수 1위 narrative가 실제로 무관한
      "배송지"였고 컨텍스트를 그거 하나로 줄이자 정답인 "장바구니 최대 보관 수량" 항목이
      빠져 LLM이 "관련 지식을 찾지 못했습니다"로 답변 자체에 실패 — 즉시 되돌림. **최종
      설계**: `build_policy_context()`(LLM 컨텍스트)는 top_k=5×2 다중 후보를 그대로 유지해
      재현율을 지키고, 화면 근거 카드만 신규 `select_cited_hit()`으로 **답변 생성 후**
      역추적 — 최종 답변 텍스트와 각 후보 원문(`raw_body`)의 토큰 겹침이 가장 큰 1건만
      선택(겹치는 게 없으면 근거 없음으로 처리, 억지로 아무거나 보여주지 않음). `agent.py`는
      이 때문에 정책 인용을 LLM 스트리밍 종료 후 두 번째 `meta` SSE 이벤트로 늦게 전송.
      부수 수정: param 검색 SQL의 `ORDER BY`가 `i.id DESC`(사실상 무의미한 최신순)에서
      `ts_rank DESC`(실제 관련도순)로 교정, `ParamHit`에 `score` 필드 추가. 프론트엔드
      변경 없음(`PolicyCitationCard` 목록 렌더링이 배열 길이에 자동 적응). 실 E2E 재확인:
      온라인스토어 DB/딜리버스 DB 양쪽에서 근거가 정확히 1건만 뜨고 실제 답변 수치와
      정확히 일치. 테스트 4개 교체(총 343개)
...
- [x] **저장소 실험실 API화 + 항목 브라우저 개선 + 관리자 화면 통합(2026-09-04)** — 사용자
      피드백 3건 반영. ①`POST /api/policy/track2/run`(`service/policy/track2.py`)로 Track 2를
      매번 스크립트로 짜지 않고 버튼으로 재실행 가능하게(admin 전용, 1~2분 소요, 실행 후 임시
      네임스페이스 자동 정리) — 실행 결과가 수동 스크립트와 정확히 동일함을 실측 확인. ②`GET
      /api/policy/items`에 원문(`raw_body`) 표시 추가 + `q`를 정책명 ILIKE에서 실제
      `search_policy()`(RDB+벡터) 재사용으로 개선, `matched_via`로 매칭 경로 표시 — 단 벡터
      매칭에 임계치가 없으면 사실상 전체가 매칭돼버리는 걸 실측으로 발견(딜리버스 71개 중
      67개=94%)해 최소 유사도(0.4) 필터 추가(41%로 개선). ③관리자 화면을 "정책" 대분류 탭
      하나 + 서브탭 3개(항목 브라우저/미분류/저장소 실험실)로 재구성(`PolicyPanel.tsx`,
      `VocEmailPanel.tsx`와 동일 패턴). 실사용 중 대분류 드롭다운이 필터링된 목록에서 선택지를
      다시 뽑아 카테고리를 고르면 다른 선택지가 사라지는 자기잠식 버그도 발견·수정. 테스트
      21개 추가(총 325개), 전부 실 HTTP E2E 병행 검증
...
## 9. 실 데이터 적재 결과 (2026-09-04, 온라인스토어 DB + 딜리버스 DB)
