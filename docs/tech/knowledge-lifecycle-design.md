# 지식(rag_knowledge) 생명주기 관리 — 현황 분석 및 장기 설계 (2026-08-28 작성, 2026-09-08 §1 우선순위 실행)
...
> 2026-09-08: §5 우선순위 1~3위 + 신규 6위(피드백→리뷰 신호) 실행 완료. §2.2/§2.4가 지적한
> "병합 시 데이터 소실"/"하드 삭제 복구 불가" 위험은 더 이상 열려있지 않음 — 상세는 §6.

---

## 1. 결론 요약

- (2026-08-28 시점 기록, 아래는 그 당시 상태) 지금 시스템엔 "신규 등록이 기존 지식의
  개정본인지"를 **자동 판단하는 로직이 없다** — 사람이 코사인 유사도로 걸러진 후보를 직접
  비교해서 승인/반려/병합을 결정한다(v2.34, 아래 §2). **이 판단 자체(LLM 관계분류)는 여전히
  없음** — §6에서 실행한 건 병합/삭제의 안전장치와 스키마 선추가일 뿐, Knowledge Resolver의
  나머지 절반(LLM 자동분류)은 §5.2 판단대로 여전히 보류 상태.
...
- 처리 방식은 **승인/반려/병합** 3가지(`resolve_duplicate`, `service.py:302-364`, 라우터
  ...
  - **병합**: 기존(타겟) 지식의 `content`/`embedding`/`updated_at`을 새 내용으로 **그 자리에서 덮어씀**
    ...
    **2026-09-08 해소**: 덮어쓰기 전 `rag_knowledge_history`에 이전 content/embedding을 먼저
    적재하도록 수정(같은 트랜잭션 안에서 SELECT→INSERT→UPDATE).
...
**2026-09-08**: 컬럼은 추가됨(Phase 0, §6) — `status`엔 `deleted`도 추가됐고, `logical_document_id`
(기본값=자기 id, 마이그레이션 시 백필)/`version`/`supersedes_id`는 스키마만 있고 아직 아무 코드도
안 읽음(계획대로 "로직 없음").

### 2.4 삭제 — 하드 삭제
`delete_knowledge`/`bulk_delete_knowledge`(`service.py:151-166`)는 진짜 `DELETE FROM`. 복구 불가.
~~복구 불가.~~ **2026-09-08 해소**: `status='deleted'` UPDATE로 전환. 검색/목록이 이미
`status='active'`만 조회하므로 즉시 안 보이면서 DB에서 사라지지 않아 복구 가능.
...
| 순위 | 항목 | 위험도/가치 | 작업량 | 상태 |
|---|---|---|---|---|
| 1 | 병합 시 기존 content 이력 보존 | 위험 높음(지금도 열려있는 사고) | 작음 | ✅ 완료(2026-09-08) |
| 2 | 삭제 하드→soft-delete | 위험 높음 | 작음 | ✅ 완료(2026-09-08) |
| 3 | Phase 0 스키마 컬럼 선추가 | 낮음(지금), 나중엔 비쌈 | 작음 | ✅ 완료(2026-09-08) |
...
| 6 | 피드백→지식 리뷰 신호(신규) | few-shot 큐 방치 사례로 확인된 저평가 레버 | 중간 | ✅ 완료(2026-09-08) |
...
- [x] Phase 1(병합 이력 보존, soft-delete) — **2026-09-08 실행 완료.** 병합(merge)이 대상 지식을
      덮어쓰기 전에 `rag_knowledge_history`(content/embedding/replaced_by_knowledge_id)에
      먼저 적재하도록 `resolve_duplicate()` 수정. 삭제(`delete_knowledge`/`bulk_delete_knowledge`)는
      하드 DELETE → `status='deleted'` UPDATE로 전환(검색/목록 쿼리가 이미 `status='active'`만
      보므로 별도 필터링 없이 즉시 안 보이면서 복구는 가능). 실 시나리오로 검증(동일 내용 2건
      등록→중복탐지→병합→history 테이블에 옛 내용 보존 확인, 삭제 후 status 확인+목록 미노출 확인).
- [x] Phase 0(스키마 선추가) — **2026-09-08 실행 완료.** `rag_knowledge`에
      `logical_document_id`(신규 행은 다음 마이그레이션에서 자동 백필=자기 id)/`version`(기본 1)/
      `supersedes_id`/`embedding_model`(신규 등록분부터 채움, 과거분은 NULL="모델 불명" 유지)/
      `quality_score`/`reviewed_at`/`owner` nullable 컬럼 추가. 로직은 최소한만(embedding_model
      기록) — 문서 원칙대로 "지금은 스키마만" 유지.
- [x] **피드백→지식 리뷰 신호** (이번 세션에서 새로 나온 항목, §3/§5.2의 "저평가된 레버") —
      나빠요 피드백이 message_id와 함께 오면, 그 답변이 실제로 근거로 삼았던 지식 전체
      (`ops_message.results`)를 `rag_knowledge_review_flag`에 리뷰 후보로 남긴다(자동 감점 아님,
      사람이 볼 큐). 지금까지는 프론트가 넘긴 지식 하나만 감점됐고 나머지 근거는 신호가 전혀
      안 남았음. Admin > 지식 베이스에 "리뷰 신호" 서브탭 신설(카운트 배지 포함)으로 노출.
      few-shot 승인 큐(활성1/대기12, 11일째 방치 확인됨)와 같은 실수를 반복하지 않으려면 이
      큐도 실제로 들여다보는 습관이 붙는지 지켜볼 것 — 안 쓰이면 §5.2와 같은 논리로 재검토.
