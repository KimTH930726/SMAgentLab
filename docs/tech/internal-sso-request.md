...
남김). 프로토콜은 OIDC(OAuth 2.0 기반)로 확인됐으나, 그 외 구체적인 연동 방법은 아직
모른다 — 타 팀이 이미 이 포털과 연동한 사례가 있다고 하니, 그 팀 또는 포털을 운영하는
...
- **Device Code Flow(퍼블릭 클라이언트) 요청하지 말 것** — 피싱 리스크로 보안팀이 비권장
  사유 거절한 전례. 처음부터 Authorization Code Flow(+PKCE)로 요청.
- **Client ID와 Object ID를 혼동하지 말 것** — 로그인에 쓰는 값은 Application (client) ID.
- **JSM 회신의 "SecretKey ID"/"SecretKey Text" 라벨이 실제 값과 반대로 붙어 있었던 전례** —
  `~` 포함된 복잡한 문자열이 실제 Secret Value, GUID 형식이 Secret ID.
- 리다이렉트 URL은 완전히 일치해야 함(오타/트레일링 슬래시 등).
- 인증서/시크릿 만료 기간은 처음부터 명시해서 요청(기본값으로 남기면 나중에 재확인 필요).
